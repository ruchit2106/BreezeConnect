"""Monthly option-expiry calculation for NSE single-stock options."""
...
def years_to_expiry(snapshot_dt: datetime, expiry_day: date) -> float:
    """Time to expiry in years (Act/DAYS_PER_YEAR), from a snapshot datetime to
    expiry at MARKET_CLOSE_TIME IST. Floored for stability."""
    close = datetime.strptime(Config.MARKET_CLOSE_TIME, "%H:%M:%S").time()
    expiry_dt = datetime.combine(expiry_day, close)
    seconds = (expiry_dt - snapshot_dt).total_seconds()
    years = seconds / (Config.DAYS_PER_YEAR * 24 * 3600)
    return max(years, Config.MIN_YEARS_TO_EXPIRY)
